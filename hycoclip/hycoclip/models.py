...
class MERU(CLIPBaseline):
    ...
    def encode_image(self, images: torch.Tensor, project: bool):
        ...
        # mapping function (focus on this!) (III)
        if project:
...
class HyCoCLIP(MERU):
    ...
    def forward(
        ...
        # shape: (batch_size, embed_dim)
        # project image and text to hyperbolic space
        # note: embedding is first getting features in euclidean space, then projecting them to hyperbolic space (III)
        image_feats = self.encode_image(images, project=True)
        text_feats = self.encode_text(tokens, project=True)
        ######
